...
def get_parser() -> ArgumentParser:
    parser = ArgumentParser(description='Dark Experience Replay '
...
class DerBi(ContinualModel):
    ...
    def build_buffer(self, dataset):
        ...
        if self.task > self.args.n_task_per_seq - 1:
            # shrink buffer
            buf_x, buf_y, _, buf_tl = self.buffer.get_all_data()
            self.buffer.empty()
            ...
            for i, data in enumerate(dataset.train_loader):
                ...
                if examples_per_task - counter > 0:
                    self.buffer.add_data(examples=not_aug_inputs[:(examples_per_task - counter)],
                                     ...
                                     logits=self.net(not_aug_inputs[:(examples_per_task - counter)]).data,
